...
describe("ExecutionVerifier", function () {
  ...
  describe("Hash Storage", function () {
    it("Should allow user with STORE_ROLE to store hash with user address", async function () {
      ...
      await executionVerifier.write.storeHash([testHash, userAddress], { account: admin.account.address });
    ...
    it("Should not allow user without STORE_ROLE to store hash", async function () {
      ...
      await expect(
        executionVerifier.write.storeHash([testHash, userAddress], { account: user1.account.address })
      ).to.be.rejected;
    });
    ...
    it("Should revert when trying to store the same hash twice", async function () {
      ...
      await expect(
        executionVerifier.write.storeHash([testHash, userAddress], { account: admin.account.address })
    ...
    it("Should allow storing different hashes with different user addresses", async function () {
      ...
      await executionVerifier.write.storeHash([hash1, userAddress1], { account: admin.account.address });
      await executionVerifier.write.storeHash([hash2, userAddress2], { account: admin.account.address });
    ...
    it("Should store hash with deployer address when userAddress is zero", async function () {
      const { executionVerifier, admin } = await loadFixture(deployExecutionVerifierFixture);

      const testHash = "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef";
      const zeroAddress = "0x0000000000000000000000000000000000000000";

      await executionVerifier.write.storeHash([testHash, zeroAddress], { account: admin.account.address });
  ...
  describe("Batch Hash Storage", function () {
    it("Should allow user with STORE_ROLE to store multiple hashes in batch", async function () {
      ...
      await executionVerifier.write.storeHashBatch([hashes, userAddresses], { account: admin.account.address });

      // Verify all hashes were stored correctly
      for (let i = 0; i < hashes.length; i++) {
        const storedAddress = await executionVerifier.read.isStored([hashes[i]]);
        expect(storedAddress.toLowerCase()).to.equal(userAddresses[i].toLowerCase());
      }
    });
    ...
    it("Should not allow user without STORE_ROLE to store hashes in batch", async function () {
      ...
      await expect(
        executionVerifier.write.storeHashBatch([hashes, userAddresses], { account: user1.account.address })
      ).to.be.rejected;
    });

    it("Should revert when arrays length don't match", async function () {
      const { executionVerifier, admin, user1 } = await loadFixture(deployExecutionVerifierFixture);
      ...
      const userAddresses = [user1.account.address]; // Only one address for two hashes

      await expect(
        executionVerifier.write.storeHashBatch([hashes, userAddresses], { account: admin.account.address })
    ...
    it("Should revert when trying to store a hash that already exists in batch", async function () {
      ...
      await executionVerifier.write.storeHash([existingHash, user1.account.address], { account: admin.account.address });
    ...
    it("Should store large batches efficiently", async function () {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        hashes.push(hash);
        userAddresses.push(i % 2 === 0 ? user1.account.address : admin.account.address);
      }
  ...
  describe("Role Management", function () {
    ...
    it("Should allow admin to grant STORE_ROLE to multiple addresses in batch", async function () {
      ...
      await executionVerifier.write.grantStoreRoleBatch([addresses], { account: admin.account.address });
    ...
    it("Should not allow non-admin to grant STORE_ROLE in batch", async function () {
      ...
      await expect(
        executionVerifier.write.grantStoreRoleBatch([addresses], { account: user1.account.address })
      ).to.be.rejected;
    });
    ...
    it("Should allow admin to revoke STORE_ROLE from multiple addresses in batch", async function () {
      ...
      // Then revoke it
      await executionVerifier.write.revokeStoreRoleBatch([addresses], { account: admin.account.address });
    ...
    it("Should not allow admin to revoke STORE_ROLE from self", async function () {
      ...
      await expect(
        executionVerifier.write.revokeStoreRoleBatch([[admin.account.address]], { account: admin.account.address })
    ...
    it("Should not allow admin to revoke STORE_ROLE from another admin", async function () {
      ...
      await expect(
        executionVerifier.write.revokeStoreRoleBatch([[user1.account.address]], { account: admin.account.address })
      ).to.be.rejectedWith("CannotRevokeFromAdmin");
    });
  });
  ...
  describe("Batch Size Limits", function () {
    ...
    it("Should allow batch operations up to MAX_BATCH_SIZE", async function () {
      const { executionVerifier, admin } = await loadFixture(deployExecutionVerifierFixture);

      // Create array with exactly MAX_BATCH_SIZE addresses
      const addresses: string[] = [];
      for (let i = 0; i < 20; i++) {
        addresses.push(`0x${i.toString().padStart(40, '0')}`);
      }

      // Should not revert when at max size
      await executionVerifier.write.grantStoreRoleBatch([addresses], { account: admin.account.address });
    });
    ...
    it("Should revert when batch size exceeds MAX_BATCH_SIZE", async function () {
      ...
      await expect(
        executionVerifier.write.grantStoreRoleBatch([addresses], { account: admin.account.address })
    ...
    it("Should revert storeHashBatch when batch size exceeds MAX_BATCH_SIZE", async function () {
      ...
      for (let i = 0; i < 21; i++) {
        ...
        userAddresses.push(`0x${i.toString().padStart(40, '0')}`);
      }
    ...
    it("Should revert hasStoreRoleBatch when batch size exceeds MAX_BATCH_SIZE", async function () {
      ...
      // Should revert when exceeding max size
      await expect(
        executionVerifier.read.hasStoreRoleBatch([addresses])
      ).to.be.rejectedWith("BatchSizeTooLarge");
    });
  });
});
