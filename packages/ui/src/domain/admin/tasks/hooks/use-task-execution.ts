...
export function useTaskExecution() {
  ...
  const uploadProofImage = async (
    file: File,
    taskId: string
  ): Promise<UploadProofImageResponse | null> => {
    try {
      setUploading(true);
  ...
  const executeTask = async (
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ["public-tasks"] });
  ...
  const batchExecuteTasks = async (
    tasks: ExecuteTaskData[]
  ): Promise<BatchExecuteResponse | null> => {
    try {
      setExecuting(true);
      ...
      if (summary.failed > 0) {
        toast.warning(
          `${summary.successful} tasks executed successfully, ${summary.failed} failed`
        );
      } else {
